...
## 0. このフォークについて
このリポジトリはnodchipさんのOnlineJudgeHelperをフォークしたのち、python2向けだったのをpython3に対応させるものです。2019-01-18現在、Atcoderの入出力例ダウンロード(-d)・コンパイル＆解答チェック(--check)・提出(-s)のみ対応しています。今後増やしていく予定です。ユーザから見た使い方は変わっていません（未実装部が動かない以外は）。
